...
sample(breakfasts, 1, replace=FALSE)
sample(dinner, 3, replace=FALSE)
chosen.recipes = c(sample(breakfasts, 1, replace=FALSE), sample(dinner, 3, replace=FALSE))
...
for(i in 1:nrow(df)){
  ...
  all.ingredients[[i]]=ingredients=html_elements(ingredients.section, "li")
  # get instructions
  instruction.section=html_elements(recipe.item, "#instructions")
  all.instructions[[i]]=instructions=html_elements(instruction.section, "ol")
  # get image
  all.images[[i]]=image.element = html_element(recipe.item, ".figure-img")
}
```
