...
for(i in 1:nrow(df)){
  ...
  # get image
  all.images[[i]]=html_element(recipe.item, ".figure-img")
}
```
...
```{r}
#| include: false
recipe.item <- read_html(df$recipe.link[1])
ingredients.section=html_element(recipe.item, "#ingredients")
bf.ingredients=html_elements(ingredients.section, "li")
instruction.section=html_elements(recipe.item, "#instructions")
bf.instructions=html_elements(instruction.section, "li")
bf.image=html_element(recipe.item, ".figure-img")
```

```{r}
#| echo: false
#| results: 'asis'
formatted_list <- paste("-", html_text(bf.ingredients))
cat(formatted_list, sep = "\n")
```