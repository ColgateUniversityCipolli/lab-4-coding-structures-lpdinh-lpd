...
df$total.score <- rep(NA, nrow(df))
  ...
  x2 = df[i,]$x2
  x3 = df[i,]$x3
  df$total.score[i] = score(x1,x2,x3)
}
(df$total.score)
```
...
**Solution**
```{r}
(instructions=html_elements(instruction.section, "ol"))
```
...
**Solution**
```{r}
paste("https://www.website.com",html_attr(image.element, "src"), sep="")
```
...
cat(paste('<img src="', paste("https://www.website.com",html_attr(image.element, "src"), sep=""), '" style="width: 50%; height: auto;">'))
```
